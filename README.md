...
System initializes LCD, RTC, keypad, buzzer, and LED
User sets:
Time
Date
Day
User configures medicine alert time
RTC continuously compares current time with medicine time
On match:
Buzzer turns ON
LED turns ON
LCD shows alert
User acknowledges alert using keypad
🧠 Key Files Explained
DOSE_GAURDIAN.c
Main entry point
...
LCD initialization
Custom LCD characters loaded (arrows)
RTC initialization
Keypad initialization
Startup message display
Enter main super-loop
User configuration mode via keypad
Store time/date/day values
Continuous RTC monitoring
Trigger alert on time match
User acknowledgment clears alert
📸 Hardware & Simulation Outputs
The following results were captured during testing:

LCD Time & Date Configuration Screen
image ## Medicine Alert Trigger Screen Screenshot 2025-12-29 164007 ## Buzzer and LED ON Indication image
